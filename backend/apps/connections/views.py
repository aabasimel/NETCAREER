...
class ConnectionViewSet(viewsets.ModelViewSet):
    ...
    def rejected(self, request):
        ...
        rejected_connections = Connection.objects.filter(
            to_user=request.user, status="rejected"
        )
    ...
    def accept(self, request, pk=None):
        ...
        Profile.objects.filter(user=connection.from_user).update(
            connection_count=F("connection_count") + 1
        )
        Profile.objects.filter(user=connection.to_user).update(
            connection_count=F("connection_count") + 1
        )
        ...
        try:
            ...
            if my_connections_ids:
                connections_my_connections = Connection.objects.filter(
                    Q(from_user_id__in=my_connections_ids)
                    | Q(to_user_id__in=my_connections_ids),
                    status="accepted",
                ).values_list("to_user_id", "from_user_id")
                ...
                suggested_connections = (
                    ...
                    .exclude(user_id__in=my_connections_ids)
                    .exclude(user_id=request.user.user_id)[:20]
                )

                serializer = UserProfileSerializer(
                    suggested_connections, many=True, context={"request": request}
                )
                return Response(serializer.data, status=status.HTTP_200_OK)
            else:
