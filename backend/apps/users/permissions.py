...
class IsJobSeeker(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        return bool(
            request.user
            and request.user.is_authenticated
            and getattr(request.user, "role", None) == "jobseeker"
        )
    ...
    def has_permission(self, request, view):
        ...
        return (
            Connection.objects.filter(
                status="accepted", from_user=request.user, to_user_id=user_id
            ).exists()
            or Connection.objects.filter(
                status="accepted", from_user_id=user_id, to_user=request.user
            ).exists()
        )
